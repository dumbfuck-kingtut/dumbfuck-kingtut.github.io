...
void main()
{
    ...
    if (((channelShift + dispersion) * intensity) < DELTA) {
		...
		outColour = extractRed(texture2D(gm_BaseTexture, mirror(offsetCoords + vec2(shiftFactors.r, 0.0)))) + extractBlue(texture2D(gm_BaseTexture, mirror(offsetCoords + vec2(/*-shiftFactors.g*/0, 0.0)))) + extractGreen(texture2D(gm_BaseTexture, mirror(offsetCoords)));
	}
	
    // Add noise	
    outColour.rgb *= (vec3(.75, 0.0, 0.0) * randomHiFreq.gab * intensity * noiseLevel) + 1.0;
